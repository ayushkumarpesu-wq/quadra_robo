...
class CmdManager_ROS():
    ...
    def _joy_cmd_callback(self, msg):
        ...
        if self.cmd.mode.start:
            self.cmd.body.height = 120  # standing value
            self.cmd.mode.walk = msg.states[1] if hasattr(self, 'is_standing') and self.is_standing else False
            ...
            self.cmd.body.height = 80  # lying/rest value
            self.cmd.mode.walk = False
            self.cmd.gait.step_len[0] = 0
            self.cmd.gait.step_len[1] = 0
            self.cmd.gait.swing_step_h = 0
